...
\item \textbf{Searching a Graph}
	\begin{enumerate}
		\item
			...
			Assume you are provided with a "Graph" class with a getNeighbors( int vertex ) method, which returns an ArrayList<Integer> representing the numbers corresponding to neighboring vertices. \\
			Assume "visited" is a Collection keeping track of all visited vertices. \\
			(Note: your algorithm should return a boolean value, not an actual path!)
			
			def hasPathToRec( Graph g, int start, int goal, HashMap\textless Integer, boolean\textgreater visited )\{
\begin{answer}
\begin{lstlisting}
	if( start == goal ){
		return true;
...
\}
		...
		\item
			...
			def hasPathToIter( Graph g, int start, int goal, HashMap\textless Integer, boolean\textgreater visited )\{
\begin{answer}
\begin{lstlisting}
...
\begin{answer}
User Datagram Protocol. We use UDP when you are managin a tremendous amount of state (esp weather data, video transmission) 
\end{answer}

\item Which one does Stream socket use for data transmission? TCP (or) UDP?

\begin{answer}
TCP. 
\end{answer}

\item Which one does Datagram socket use for data transmission? TCP (or) UDP?

\begin{answer}
UDP
\end{answer}

\item What is datagram?
...
\begin{answer}
the endpoints of logical connections between two hosts and can be used to send and receive data.
\end{answer}
