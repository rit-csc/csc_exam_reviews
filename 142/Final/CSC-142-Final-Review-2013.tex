...
\item \textbf{Searching a Graph}
	\begin{enumerate}
		\item
			...
			Assume you are provided with a "Graph" class with a getNeighbors( int vertex ) method, which returns an ArrayList\textless Integer\textgreater representing the numbers corresponding to neighboring vertices. \\
			Assume "visited" is an ArrayList keeping track of all visited vertices. \\
			(Note: your algorithm should return a boolean value, not an actual path!)
\begin{verbatim}
boolean hasPathToRec(Graph g, int start, int goal, ArrayList<Integer> visited) {
\end{verbatim}
...
\begin{verbatim}
boolean hasPathToIter(Graph g, int start, int goal, ArrayList<Integer> visited) {
\end{verbatim}	
