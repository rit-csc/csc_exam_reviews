...
\item Why does Dijkstra's algorithm not work correctly on graphs with negative
    ...
    fever. If there were another route to that node, which had not yet been
    explored (and contained a net negative weight) Dijkstra's algorithm would
    return a suboptimal path. In the following example, the final edge weights
    should be \{A:0,B:1,C:3\} but they are \{A:0,B:2,C:3\}
