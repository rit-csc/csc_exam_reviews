...
\item
\label{listdupes}
Write a Python function that, given a Python list, returns a list of all of the elements which are in the list more than once. (e.g.
...
def find_recur(lst):
	...
	if lst[0] not in seen:
		seen.add(lst[0])
		return find_recursive(lst[1:], seen)
	return [lst[0]]+find_recursive(lst[1:], seen)
\end{lstlisting}
\textit{or}
\begin{lstlisting}
def find_tail(lst, result=[], seen=set()):
	if len(lst)==0:
		return result
	if lst[0] not in seen:
		seen.add(lst[0])
