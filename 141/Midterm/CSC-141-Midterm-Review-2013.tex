...
\begin{document}
\noindent{\Large CS-141 Midterm Exam Review \hfill \today}
...
def makeRunner(name,age,fastestTime):
    ...
    runner.name = name
    runner.age = age
    runner.fastestTime = fastestTime
    return runner
\end{lstlisting}
\end{answer}

\item The glorious leader has decided that, on this day, no runner named \texttt{Joe} may win
gold. Given a list of track runner objects, write the function \texttt{aWinnerIsYou(r)} that finds the runner in the list \texttt{r} with the fastest time who's name is not Joe. 
...
\item
In the game Black and White, the player is faced with a row of identical double-sided chips.
...
If every flip takes one ``action,'' write a function \texttt{bwMoves} that, given a starting pattern and target pattern as strings, returns the minimum number of actions required to get them to match?
