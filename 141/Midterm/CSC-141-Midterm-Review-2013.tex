...
\newpage
\item How can you use a stack to determine whether an input string has properly
      balanced parentheses, brackets and braces? (e.g. ``[\{\}]'' is accepted, but
      ``(()'' and ``[(])'' are not)?
      Assume that you are provided with a Stack class that has push(), pop(), and top() member functions.
...
def bwMoves(start, target):
	...
	for index in range(len(start)):
		if start[index]==target[index]:
			if first!=index:	# For each flip, you are going to flip up to (but not including)
				actions+=1		# the index pointer.  If first==index, you are flipping a set of
			first=index+1		# zero elements.  There is nothing to flip.
								# ie:  There were two no-flips in a row.
	if start[-1]!=target[-1]:
		actions+=1
	return actions
	
NOTE: If you encounter flip condition, you want to flip everything AFTER the start pointer.
\end{lstlisting}
\end{answer}
